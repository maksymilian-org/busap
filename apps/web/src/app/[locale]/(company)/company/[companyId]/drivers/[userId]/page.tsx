...
export default function CompanyDriverDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [profileData, weekData, historyData] = await Promise.all([
        ...
        api.fetch<any>(`/trips?companyId=${companyId}&driverId=${userId}&fromDate=${from.toISOString()}&toDate=${to.toISOString()}`),
        api.fetch<any>(`/trips?companyId=${companyId}&driverId=${userId}&limit=20`),
      ]);
  ...
  if (!profile) {
    return (
      <div className="text-center py-12">
        ...
        <Button asChild className="mt-4"><Link href={`/company/${companyId}/drivers`}>{t('back')}</Link></Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild>
          <Link href={`/company/${companyId}/drivers`}><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
